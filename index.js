...
window.onload = function initial() {
  ...
  var catNames = function () {
    ...
    for (const [key, value] of Object.entries(categories)) {
      console.log(key);
      text += `<a id='cat' class = "a" value = '${key}' onclick="update_word_list('${key}')" >${key}</button>`;
    }
    cats.innerHTML += text;
  };
  ...
  if (!answer.includes(letter)) {
    ...
    if (incorrect == maxIncorrect) {
      ...
      document.getElementById(
        "gameover"
      ).innerText = `Game Over, answer was ${answer} `;
    }
  }
}
...
document.querySelector("#catName").addEventListener("click", function () {
  document.querySelector(".popup").style.display = "none";
});
